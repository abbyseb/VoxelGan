#!/usr/bin/env python3
"""Fine-tune Dan UNetCRB warp-D model on P2 Elastix pairs."""
...
def main():
    ...
    ap.add_argument("--train_dir", default=str(PARENT / "P2" / "data" / "train"))
    ap.add_argument("--val_dir", default=str(PARENT / "P2" / "data" / "val"))
    ap.add_argument("--init_g", default=str(ROOT / "weights" / "dans_crb_warp_d_generator.pth"))
    ap.add_argument("--init_d", default=str(ROOT / "weights" / "dans_crb_warp_d_discriminator.pth"))
    ap.add_argument("--out_prefix", default="dans_crb_p2_warp_d_finetune")
    ...
    generator = UNetCRB(im_size=args.im_size, n_phases=10).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if v_mean < best_val:
            torch.save(generator.state_dict(), str(ROOT / "weights" / f"{args.out_prefix}_generator.pth"))
            torch.save(discriminator.state_dict(), str(ROOT / "weights" / f"{args.out_prefix}_discriminator.pth"))
            best_val = v_mean
