#!/usr/bin/env python3
"""Clinical zero-shot QC for Grid160 G160-A1 FOV checkpoints.

  cd PopulationStudy/ClinicalExperiments/Grid160/Experiment1
  PYTHONPATH=. python scripts/qc_clinical.py --norm both --panel-pairs all --gpu 0
"""
...
CE1 = E1.parents[1] / "Experiment1"
IE6 = E1.parents[3] / "InitialExperiments" / "Experiment6"
...
def run_panels_only(arm, arch, scan, g, device, dvf_l1, norm_fn, norm_name, panel_pairs, force, ckpt_path):
    ...
    out_dir = E1 / ARCH_DIR[arch] / "plots" / f"qc_{vendor}" / arm / norm_name / patient / scan
...
def main():
    ...
    ap.add_argument("--arms", default="a1_full")
    ...
    dvf_l1 = DVFLoss()
    print(
        f"Grid160-A1 QC  device={device}  arms={arms}  arches={arches}  norms={norms}"
    ...
    if args.panels_only:
        for arm in arms:
            for arch in arches:
                ...
                ckpt_path = E1 / ARCH_DIR[arch] / "weights" / ckpt_name
    ...
    print("\n======== GRID160 A1 CLINICAL SUMMARY ========", flush=True)
    for arm in all_summ:
