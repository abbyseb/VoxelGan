#!/usr/bin/env python3
"""Experiment1 QC with Experiment5 intensity settings (µ air/water).

Writes panels under Experiment1/EXPT5_SETTINGS/{Encoder,Decoder,Both}CRB/plots/qc_{varian,elekta}/.
...
DEFAULT_CKPTS = {
    "encoder": "EncoderCRB/weights/crb_enc_mse_full_spare_generator.pth",
    "decoder": "DecoderCRB/weights/crb_dec_mse_full_spare_generator.pth",
    "both": "BothCRB/weights/crb_both_mse_full_spare_generator.pth",
}
...
def run_one(arch: str, scan: str, g, device, dvf_l1):
    ...
    out_dir = OUT_ROOT / ARCH_DIR[arch] / "plots" / f"qc_{vendor}" / patient / scan
    ...
    summary = {
        ...
        "arch": arch,
        "norm": "mu_air_water",
        "settings": "EXPT5_SETTINGS",
        "ckpt": str(E1 / DEFAULT_CKPTS[arch]),
        "n_pairs": len(rows),
...
def main():
    ...
    dvf_l1 = DVFLoss()
    print(f"E1×E5-settings QC  device={device}  out={OUT_ROOT}", flush=True)

    all_summ = {}
    for arch in arches:
        ckpt = E1 / DEFAULT_CKPTS[arch]
        if not ckpt.is_file():
        ...
        all_summ[arch] = {}
        for scan in scans:
            s = run_one(arch, scan, g, device, dvf_l1)
            all_summ[arch][scan] = {
                k: s[k]
                for k in (
                    "mean_L1",
    ...
    (OUT_ROOT / "summary_all.json").write_text(json.dumps(all_summ, indent=2) + "\n")
    (OUT_ROOT / "metrics.tsv").write_text("\n".join(lines) + "\n")
    print(f"wrote {OUT_ROOT / 'summary_all.json'}", flush=True)
