#!/usr/bin/env python3
"""Simple zero-shot QC: full-SPARE model on Clinical Elekta DVF library.

  cd PopulationStudy/ClinicalExperiments/Experiment1
  PYTHONPATH=. python scripts/qc_elekta.py \\
    --scan CE_P1_V_01 \\
    --ckpt DecoderCRB/weights/crb_dec_mse_full_spare_generator.pth
"""
...
def main():
    ...
    ap.add_argument("--arch", default="decoder", choices=sorted(ARCH))
    ap.add_argument("--scan", default="CE_P1_V_01")
    ...
    data_dir = Path(args.data_root or E1 / "data" / args.scan / "all")
    # Parse patient from CE_P1_V_01 → P1
    m = re.match(r"CE_(P\d+)_", args.scan)
    ...
    ckpt = E1 / (args.ckpt or DEFAULT_CKPTS[args.arch])
    if not ckpt.is_file():
    ...
    print(
        f"[{args.arch}|elekta] scan={args.scan} pairs={len(pair_stems)} panels={len(panel_pairs)} "
        ...
        for i, stem in enumerate(pair_stems, 1):
            ...
            if stem in panel_pairs:
                save_panel(
                    ...
                    f"{args.scan} {stem} (zero-shot {args.arch})",
                    l1, cos, k,
                )
    ...
    print(
        f"\n[{args.arch}|elekta] Directed mean: L1={summary['mean_L1']:.3f}  "
