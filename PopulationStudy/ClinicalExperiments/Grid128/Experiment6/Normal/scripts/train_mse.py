"""Train CRB for ClinicalExperiments Experiment6/Normal (linear phase, 128³).

Same full-SPARE MSE recipe as Experiment1, but train/val crops are full **128³**
(volumes are already 128³ → one full volume per pair, not 64³ patches).
Phase encoding: linear cond_dim=2 (InitialExperiments nets).
"""
...
EXP = Path(__file__).resolve().parents[1]  # .../Experiment6/Normal
# Normal → Experiment6 → ClinicalExperiments → PopulationStudy
IE1 = EXP.parents[2] / "InitialExperiments" / "Experiment1"
if str(IE1) not in sys.path:
    sys.path.insert(0, str(IE1))
assert IE1.is_dir(), f"missing IE1 at {IE1}"
...
ARCH = {
    "encoder": {
        ...
        "filename": "crb_enc_mse_linear_full128",
        "label": "CRB=encoder | linear | train 128³",
    },
    "decoder": {
        "cls": UNetCRBDecoder,
        "run_dir": EXP / "DecoderCRB",
        "filename": "crb_dec_mse_linear_full128",
        "label": "CRB=decoder | linear | train 128³",
    },
    "both": {
        "cls": UNetCRBBoth,
        "run_dir": EXP / "BothCRB",
        "filename": "crb_both_mse_linear_full128",
        "label": "CRB=both | linear | train 128³",
    },
}
...
def train_one(arch: str, epochs: int, lr: float, gpu: int, seed: int | None):
    ...
    # Full packed volume (128³). patches_per_pair=1 — crop is identity when size matches.
    im_size = 128
    n_phases = 10
    ...
    # IE1 linear nets may not set .cond_dim; check first Linear in CRB
    w = next(p for n, p in generator.named_parameters() if n.endswith("fc.0.weight"))
    cond_dim = int(w.shape[1])
    assert cond_dim == 2, f"Normal section expects linear cond_dim=2, got {cond_dim}"
    import networks.generator_crb as _gmod
    print(
        f'[{spec["filename"]}] network_module={_gmod.__file__} cond_dim={cond_dim} im_size={im_size}',
        flush=True,
    )
