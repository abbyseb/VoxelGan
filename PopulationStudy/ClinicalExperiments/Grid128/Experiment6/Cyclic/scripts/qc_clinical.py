#!/usr/bin/env python3
"""Zero-shot clinical QC for Experiment6 Cyclic (128³ cyclic full-volume).

  cd PopulationStudy/ClinicalExperiments/Experiment6/Cyclic
  PYTHONPATH=. python scripts/qc_clinical.py --arches encoder,decoder,both --norm both --no-panels --gpu 1
"""
...
DEFAULT_CKPTS = {
    "encoder": "EncoderCRB/weights/crb_enc_mse_cyclic_full128_generator.pth",
    "decoder": "DecoderCRB/weights/crb_dec_mse_cyclic_full128_generator.pth",
    "both": "BothCRB/weights/crb_both_mse_cyclic_full128_generator.pth",
}
    ...
    with torch.no_grad():
        for stem in sorted(panel_pairs):
            ...
            save_panel(
                ...
                f"{scan} {stem} (E6 Cyclic {arch} · {norm_name})",
                l1, cos, k,
            )
            n_saved += 1
...
def main():
    ...
    dvf_l1 = DVFLoss()
    print(
        f"E6 Cyclic QC  device={device}  arches={arches}  norms={norms}"
        ...
        for norm_name in all_summ[arch]:
            for vendor, pref in (("varian", "CV_"), ("elekta", "CE_")):
                ...
                for s in subset:
                    ...
                    lines.append(
                        f"E6_Cyclic\t{arch}\t{norm_name}\t{vendor}\t{s}\t{m['mean_cos']:.4f}\t"
