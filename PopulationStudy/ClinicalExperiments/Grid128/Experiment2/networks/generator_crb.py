"""UNetCRB — ClinicalExperiments Experiment 2 (cyclic phase encoding).

Same encoder-CRB architecture as InitialExperiments E1, but phase codes are
...
Known deviations from the paper / from parent Voxel_GAN UNetFiLM:
  - Conditioning vector is 4-D cyclic (cos/sin × ref/tgt), not scalar t.
  ...
  - Output is a direct 3-channel DVF with linear activation — NO scaling-and-
    squaring (svf.py is not used). Dan 2.0 trains with MSE only (no L_smooth).
...
class ConditionalResidualBlock(nn.Module):
    ...
    def __init__(self, in_ch, out_ch, cond_dim=4):
        super().__init__()
        self.out_ch = out_ch
        # cond (4) → 32 → 16 → 2 * out_ch  (per-channel a, b)
...
class UNetCRB(nn.Module):
    ...
    def _phase_vec(self, ref_phase, target_phase):
        # θ = phase_index / n_phases → [cos 2πθ, sin 2πθ] × {ref, tgt} → (B, 4)
    ...
    def forward(self, reference_ct, ref_phase, target_phase):
        ...
        # direct DVF (linear); no SVF integration
