#!/usr/bin/env python3
"""Write E5 visual QC panels (best norm: µ air/water) for Varian + Elekta."""
...
def run_scan(scan, g, device, dvf_l1):
    ...
    out_dir = OUT / f"qc_{vendor}" / "mu_air_water" / patient / scan
    ...
    with torch.no_grad():
        for i, stem in enumerate(stems, 1):
            ...
            if i % 20 == 0:
                print(f"  [{scan}] {i}/100", flush=True)
            save_panel(
                out_dir / f"{stem}.png",
                ct_r, ct_t, warped, gt, pred, mask, z,
                f"{scan} {stem} (E5 µ air/water)",
                l1, cos, k,
            )
    ...
    print(
        f"[{scan}] cos={summary['mean_cos']:.3f} beat={summary['beat_zero_pct']:.0f}% → {out_dir}",
        flush=True,
    )
...
def main():
    ...
    g.eval()
    dvf_l1 = DVFLoss()
    print(f"E5 panels  norm=mu_air_water  device={device}", flush=True)
    for scan in [s.strip() for s in args.scans.split(",") if s.strip()]:
        run_scan(scan, g, device, dvf_l1)
