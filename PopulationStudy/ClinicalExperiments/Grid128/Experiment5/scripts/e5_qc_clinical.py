#!/usr/bin/env python3
"""E5 clinical QC: E3 cyclic+FOV weights + µ air/water inference norm.

Writes panels under Experiment5/{Encoder,Decoder,Both}CRB/plots/qc_{varian,elekta}/.

  cd PopulationStudy/ClinicalExperiments
  python Experiment5/scripts/e5_qc_clinical.py --gpu 1 --arches encoder,decoder
"""
...
DEFAULT_CKPTS = {
    "encoder": E3 / "EncoderCRB/weights/crb_enc_mse_cyclic_fov_aug_generator.pth",
    "decoder": E3 / "DecoderCRB/weights/crb_dec_mse_cyclic_fov_aug_generator.pth",
    "both": E3 / "BothCRB/weights/crb_both_mse_cyclic_fov_aug_generator.pth",
}
...
def run_one(arch: str, scan: str, g, device, dvf_l1, *, write_panels: bool, skip_existing: bool):
    ...
    out_dir = E5 / ARCH_DIR[arch] / "plots" / f"qc_{vendor}" / patient / scan
    if skip_existing and (out_dir / "summary.json").is_file():
        print(f"skip {arch} {scan} (exists)", flush=True)
    ...
    with torch.no_grad():
        for i, stem in enumerate(stems, 1):
            ...
            ct_r = norm_mu(np.load(data_dir / f"CT_{ref:02d}.npy"))
            ct_t = norm_mu(np.load(data_dir / f"CT_{tgt:02d}.npy"))
            if ref == tgt:
            ...
            if write_panels:
                save_panel(
                    ...
                    f"{scan} {stem} (E5 {arch} · µ air/water)",
                    l1, cos, k,
                )
            if i % 25 == 0:
                print(f"  [{arch}|{scan}] {i}/100", flush=True)
    ...
    print(
        f"[{arch}|{scan}] cos={summary['mean_cos']:.3f} beat={summary['beat_zero_pct']:.0f}% → {out_dir}",
        flush=True,
    )
    return summary
...
def main():
    ...
    ap.add_argument("--no-panels", action="store_true", help="metrics only (no PNGs)")
    ...
    dvf_l1 = DVFLoss()
    print(f"E5 clinical QC  device={device}  norm=mu_air_water", flush=True)
    ...
    lines = ["arch\tvendor\tscan\tcos\tL1_over_zero\tbeat"]
    print("\n======== COHORT SUMMARY (µ air/water) ========", flush=True)
    for arch, scans_d in all_summ.items():
        for vendor, pref in (("varian", "CV_"), ("elekta", "CE_")):
            subset = [s for s in scans_d if s.startswith(pref)]
            if not subset:
                continue
            cos = float(np.mean([scans_d[s]["mean_cos"] for s in subset]))
            beat = float(np.mean([scans_d[s]["beat_zero_pct"] for s in subset]))
            ratio = float(np.mean([scans_d[s]["mean_L1_over_zero"] for s in subset]))
            print(f"{arch:8s} {vendor:7s}  cos={cos:.3f}  L1/z={ratio:.3f}  beat={beat:.0f}%", flush=True)
            for s in subset:
                m = scans_d[s]
                lines.append(
                    f"{arch}\t{vendor}\t{s}\t{m['mean_cos']:.4f}\t{m['mean_L1_over_zero']:.4f}\t{m['beat_zero_pct']:.1f}"
                )

    out_json = E5 / "logs" / "e5_clinical_summary.json"
    out_tsv = E5 / "logs" / "e5_clinical_metrics.tsv"
    out_json.parent.mkdir(parents=True, exist_ok=True)
    out_json.write_text(json.dumps(all_summ, indent=2) + "\n")
    out_tsv.write_text("\n".join(lines) + "\n")
