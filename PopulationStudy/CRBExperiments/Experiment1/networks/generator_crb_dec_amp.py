"""UNetCRB-Decoder with cond_dim=3: [θ_ref, θ_tgt, log(r_p)] oracle amplitude.

CRBExperiments Experiment 1 — first ablation vs InitialExperiments Decoder-CRB.
"""
...
class ConditionalResidualBlock(nn.Module):
    def __init__(self, in_ch, out_ch, cond_dim=3):
        super().__init__()
        self.out_ch = out_ch
...
class UpCRB(nn.Module):
    def __init__(self, in_ch, skip_ch, out_ch, cond_dim=3):
        super().__init__()
...
class UNetCRBDecoderAmp(nn.Module):
    """Plain encoder, CRB decoder; predicts unit-amplitude shape field."""

    def __init__(self, im_size=128, n_phases=10, cond_dim=3):
        super().__init__()
    ...
    def _cond_vec(self, ref_phase, target_phase, log_r):
        ...
        return torch.cat([t_ref, t_tgt, log_r.float()], dim=1)
