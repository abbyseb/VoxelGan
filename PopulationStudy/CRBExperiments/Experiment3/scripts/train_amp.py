"""Train Decoder-CRB cyclic phase + oracle amp (CRBExperiments Experiment 3).

  PYTHONPATH=. python DecoderCRB/train_crb_dec_amp_mse.py --gpu 1
"""
...
def load_manifest():
    with open(E3 / "data" / "pooled" / "manifest.json") as f:
        return json.load(f)
...
def train_one(epochs: int, lr: float, gpu: int, seed: int | None):
    ...
    filename = "crb_dec_cyclic_oracle_e3"
    man = load_manifest()
    train_dir = str(E3 / "data" / "pooled" / "train")
    ...
    g = UNetCRBDecoderAmp(im_size=im_size, n_phases=10, cond_dim=5).to(device)
    ...
    print(
        ...
        f"shape-MSE + cyclic(θ) + oracle log(r_p) | cond_dim=5 | seed={seed}",
        flush=True,
    )
    print(
