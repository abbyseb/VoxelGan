"""Train cyclic + δ_tgt (inhale/exhale) + oracle amp — CRBExperiments Experiment 4.
...
def load_manifest():
    with open(E4 / "data" / "pooled" / "manifest.json") as f:
        return json.load(f)
...
def train_one(epochs: int, lr: float, gpu: int, seed: int | None):
    ...
    filename = "crb_dec_delta_cyclic_oracle_e4"
    man = load_manifest()
    train_dir = str(E4 / "data" / "pooled" / "train")
    ...
    g = UNetCRBDecoderAmp(im_size=im_size, n_phases=10, cond_dim=6).to(device)
    ...
    print(
        ...
        f"shape-MSE + cyclic(θ) + δ_tgt + oracle log(r_p) | cond_dim=6 | seed={seed}",
        flush=True,
    )
    print(
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch in train_loader:
            ...
            pred = g(
                ...
                batch["log_r_p"].to(device),
                batch["delta_tgt"].to(device),
            )
