"""UNetCRB-Decoder: cyclic phases + δ_tgt (inhale/exhale) + oracle log(r_p).

cond_dim=6:
  [cos 2πθ_ref, sin 2πθ_ref, cos 2πθ_tgt, sin 2πθ_tgt, δ_tgt, log(r_p)]
δ_tgt ∈ {+1, −1} for inhaling / exhaling at the target phase.
"""
...
class ConditionalResidualBlock(nn.Module):
    def __init__(self, in_ch, out_ch, cond_dim=6):
        super().__init__()
        self.out_ch = out_ch
...
class UpCRB(nn.Module):
    def __init__(self, in_ch, skip_ch, out_ch, cond_dim=6):
        super().__init__()
...
class UNetCRBDecoderAmp(nn.Module):
    def __init__(self, im_size=64, n_phases=10, cond_dim=6):
        super().__init__()
    ...
    def _cond_vec(self, ref_phase, target_phase, log_r, delta_tgt):
        ...
        if delta_tgt.dim() == 1:
            delta_tgt = delta_tgt.unsqueeze(-1)
        return torch.cat(
            [
    ...
    def forward(self, reference_ct, ref_phase, target_phase, log_r, delta_tgt):
        cond = self._cond_vec(ref_phase, target_phase, log_r, delta_tgt)
