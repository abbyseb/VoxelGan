"""Train Decoder-CRB A1: shape targets, log(r̂)=0 (CRBExperiments Experiment 2).
...
def load_manifest():
    with open(E2 / "data" / "pooled" / "manifest.json") as f:
        return json.load(f)
...
def train_one(epochs: int, lr: float, gpu: int, seed: int | None):
    ...
    filename = "crb_dec_a1_normonly_e2"
    man = load_manifest()
    # Prefer local pooled path (symlink) so E2 is self-contained
    train_dir = str(E2 / "data" / "pooled" / "train")
    ...
    print(
        ...
        f"shape-MSE + log(r̂)=0 (A1) | seed={seed}",
        flush=True,
    )
    print(
        f"[{filename}] A_train_mm={amp_table['A_train_mean_mm']:.3f} | "
        f"infer_scale=A_train_mean | grid={amp_table.get('grid','data_iso')} | "
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch in train_loader:
            ...
            # A1: always r̂=1 → log(r̂)=0
            log_r = torch.zeros_like(batch["log_r_p"]).to(device)
            pred = g(
                ref,
