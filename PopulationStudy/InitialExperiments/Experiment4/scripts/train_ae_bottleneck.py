#!/usr/bin/env python3
"""10-epoch probe: compressed bottleneck AE (no skips, 32-D latent).

Saves under AnatomyAE_bottleneck/ — does not overwrite the original UNet AE.
"""
...
from networks.anatomy_ae_bottleneck import BottleneckAnatomyAE
...
def train_one(epochs, lr, gpu, seed, im_size, patches_train, patches_val, latent_dim):
    ...
    model = BottleneckAnatomyAE(latent_dim=latent_dim).to(device)
    opt = optim.Adam(model.parameters(), lr=lr)
    run_dir = E4 / "AnatomyAE_bottleneck"
    os.makedirs(run_dir / "weights", exist_ok=True)
    os.makedirs(run_dir / "plots", exist_ok=True)
    ckpt = run_dir / "weights" / "anatomy_ae_bottleneck.pth"

    n = sum(p.numel() for p in model.parameters())
    print(
        f"[bottleneck_ae] device={device} params={n / 1e6:.2f}M latent={latent_dim} | "
    ...
    for epoch in range(1, epochs + 1):
        ...
        plt.title(f"bottleneck AE (latent={latent_dim}, no skips)")
        plt.savefig(str(run_dir / "plots" / "anatomy_ae_bottleneck.png"))
        plt.close()
