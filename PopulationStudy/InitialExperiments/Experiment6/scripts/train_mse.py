"""Shared MSE train loop for Experiment 6 CRB models (A0 baseline on data_iso).

Lung-masked MSE vs Elastix DVF in iso-voxels (no amplitude normalisation).
Val pairs come from train patients only.

  PYTHONPATH=. python DecoderCRB/train_crb_dec_mse.py --gpu 1
"""
...
ARCH = {
    'encoder': {
        'cls': UNetCRB,
        'run_dir': E6 / 'EncoderCRB',
        'filename': 'crb_enc_mse_pop_e6_iso',
        'label': 'CRB=encoder-only',
    },
    'decoder': {
        'cls': UNetCRBDecoder,
        'run_dir': E6 / 'DecoderCRB',
        'filename': 'crb_dec_mse_pop_e6_iso',
        'label': 'CRB=decoder-only A0 baseline iso',
    },
    'both': {
        'cls': UNetCRBBoth,
        'run_dir': E6 / 'BothCRB',
        'filename': 'crb_both_mse_pop_e6_iso',
...
def load_manifest():
    path = E6 / 'data' / 'pooled' / 'manifest.json'
    with open(path) as f:
        return json.load(f)
...
def train_one(arch: str, epochs: int, lr: float, gpu: int, seed: int | None):
    ...
    print(
        ...
        f'loss=lung-masked MSE (iso-vox) | no amp | {spec["label"]} | seed={seed}',
        flush=True,
    )
    print(
    ...
    for epoch in range(1, epochs + 1):
        ...
        # markers so epoch-1 is visible (a lone line segment needs ≥2 points)
