"""Hold-out QC for InitialExperiments Experiment 6 (A0 iso baseline).

Full-volume inference on data_iso (2 mm, 160³). Metrics in mm.

  PYTHONPATH=. python scripts/qc_pairs.py \\
    --arch decoder \\
    --ckpt DecoderCRB/weights/crb_dec_mse_pop_e6_iso_generator.pth \\
...
def save_panel(out_path, ct_r, ct_t, warped, abs_img_err, gt, pred, mask, cfg, title, l1_mm, cos):
    ...
    fig.suptitle(
        ...
        f"L1={l1_mm:.3f} mm  cos={cos:.3f}  |  data_iso 2 mm"
    )
    fig.tight_layout()
...
def main():
    ...
    ap.add_argument("--view_config", default=str(E6 / "configs" / "dvf_view_config.json"))
    ap.add_argument("--max_pairs", type=int, default=0, help="0 = all")
    ap.add_argument(
        "--panels",
        default="extreme",
        choices=["all", "extreme", "none"],
        help="extreme = one max-|u| pair per patient",
    )
    args = ap.parse_args()

    man = json.loads((E6 / "data" / "pooled" / "manifest.json").read_text())
    if args.split == "test":
    ...
    g = ARCH[args.arch](im_size=64, n_phases=10).to(device)
    g.load_state_dict(torch.load(args.ckpt, map_location=device))
    g.eval()
    dvf_l1 = loss_mod.DVFLoss()

    print(
        f"E6 QC arch={args.arch} split={args.split} pairs={len(pair_names)} "
    ...
    rows = []
    mask_cache = {}
    best_mag = {}  # patient -> (mean_mag_mm, name) for extreme panel
    ...
    with torch.no_grad():
        for i, name in enumerate(pair_names, 1):
            ...
            rows.append((tag, note, patient, l1_mm, zero_mm, ratio, cos, k))
            print(
            ...
            want_panel = args.panels == "all" or (
                args.panels == "extreme" and ref != tgt and best_mag.get(patient, (0, ""))[1] == name
            )
            # extreme: defer — write after pass; for streaming, write all non-id if panels=all
            if args.panels == "all" and (args.max_pairs > 0 or True):
                save_panel(
                    out_dir / f"{tag}.png",
    ...
    # Extreme panels: second pass for those pairs only
    ...
    with open(out_dir / "metrics.tsv", "w") as f:
        f.write("pair\tnote\tpatient\tL1_mm\tL1_zero_mm\tL1_over_zero\tcos\tk\n")
        for tag, note, patient, l1, zero, ratio, cos, k in rows:
            f.write(
