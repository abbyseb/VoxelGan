"""Dan 2.0 Both-CRB: UNetCRBBoth + lung-masked MSE only (no D).

CRB on encoder + bottleneck + decoder. Same leave-outs as Encoder/Decoder CRB.

  PYTHONPATH=.. python train_crb_both_mse.py --held_out 5,9 --run_dir LeaveOut_5_9
"""
...
DAN20 = BOTH_ROOT.parent
sys.path.insert(0, str(DAN20))
sys.path.insert(0, str(BOTH_ROOT))

from generator_crb_both import UNetCRBBoth
...
def main():
    ...
    generator = UNetCRBBoth(im_size=im_size, n_phases=n_phases).to(device)
    mse_loss = DVFMSELoss()
    ...
    print(
        f'[{filename}] device={device} | UNetCRBBoth params={n_g / 1e6:.2f}M | '
        f'loss=lung-masked MSE | no D | CRB=encoder+decoder | held_out_1idx={held_1idx}'
    )
    print(
