"""Full-volume QC for Experiment 3 CRB models on hold-out patients.
...
Metrics = lung-masked L1 + cosine vs Elastix (same as Dan 2.0 QC).
  ...
  PYTHONPATH=.. python scripts/qc_pairs.py \\
    --arch encoder \\
    --ckpt EncoderCRB/weights/crb_enc_mse_pop_e3_generator.pth \\
...
def main():
    ...
    ap.add_argument(
        '--view_config',
        default=str(E3 / 'configs' / 'dvf_view_config.json'),
    )
    ap.add_argument('--max_pairs', type=int, default=0, help='0 = all')
    args = ap.parse_args()

    man = json.loads((E3 / 'data' / 'pooled' / 'manifest.json').read_text())
    if args.split == 'test':
