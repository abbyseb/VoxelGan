"""Hold-out QC for IsoExperiments E2 (Enc/Dec/Both on data_iso).

Full-volume inference on data_iso (2 mm, 160³). Metrics in mm.

  PYTHONPATH=. python scripts/qc_pairs.py \\
    --arch decoder \\
    --ckpt DecoderCRB/weights/crb_dec_mse_iso_e2_generator.pth \\
...
def main():
    ...
    ap.add_argument("--view_config", default=str(E2 / "configs" / "dvf_view_config.json"))
    ...
    man = json.loads((E2 / "data" / "pooled" / "manifest.json").read_text())
    if args.split == "test":
    ...
    print(
        f"Iso-E2 QC arch={args.arch} split={args.split} pairs={len(pair_names)} "
