...
Copied from Experiment2 for PopulationStudy/Experiment4.
E4: pretrained AnatomyEncoder is loaded into enc1/down2–5 and frozen.
"""
...
class UNetCRBDecoder(nn.Module):
    ...
    def load_anatomy_encoder(self, path):
        from networks.anatomy_ae import load_encoder_state_dict

        sd = load_encoder_state_dict(path)
        missing, unexpected = self.load_state_dict(sd, strict=False)
        enc_missing = [k for k in missing if k.split(".")[0] in self._encoder_names()]
        if enc_missing or unexpected:
            raise RuntimeError(
                f"anatomy encoder load mismatch missing={enc_missing[:8]} "
    ...
    @staticmethod
    def _encoder_names():
        return ("enc1", "down2", "down3", "down4", "down5")

    def freeze_encoder(self):
        names = self._encoder_names()
