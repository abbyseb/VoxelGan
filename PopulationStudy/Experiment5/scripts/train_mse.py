"""Shared MSE train loop for Experiment 5 CRB models (pretrained anatomy).
...
DEFAULT_ANATOMY = E5.parent / "Experiment4" / "AnatomyAE" / "weights" / "anatomy_encoder.pth"

ARCH = {
    "encoder": {
        "cls": UNetCRB,
        "run_dir": E5 / "EncoderCRB",
        "filename": "crb_enc_mse_pop_e5",
        "label": "CRB=encoder + frozen anatomy side-branch",
        "side_branch": True,
    },
    "decoder": {
        "cls": UNetCRBDecoder,
        "run_dir": E5 / "DecoderCRB",
        "filename": "crb_dec_mse_pop_e5",
...
def load_manifest():
    path = E5 / "data" / "pooled" / "manifest.json"
    with open(path) as f:
        return json.load(f)
