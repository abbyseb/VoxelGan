"""Dan 2.0 Decoder-CRB+Bot: UNetCRBDecoderBot + lung-masked MSE only (no D).

Usage (from this directory):
  PYTHONPATH=.. python train_crb_dec_bot_mse.py --held_out 5,9 --run_dir LeaveOut_5_9
"""
...
def main():
    ...
    generator = UNetCRBDecoderBot(im_size=im_size, n_phases=n_phases).to(device)
    mse_loss = DVFMSELoss()
    ...
    print(
        f'[{filename}] device={device} | UNetCRBDecoderBot params={n_g / 1e6:.2f}M | '
        f'loss=lung-masked MSE | no D | CRB=bottleneck+decoder | held_out_1idx={held_1idx}'
    )
    print(
