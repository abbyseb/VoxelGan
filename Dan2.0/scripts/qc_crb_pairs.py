"""Full-volume QC for Dan 2.0 UNetCRB (MSE-trained). Metrics = L1 + cos vs Dan."""
...
def main():
    ...
    ap.add_argument('--ckpt', default=str(ROOT / 'weights' / 'dans_crb_mse_generator.pth'))
    ap.add_argument('--data_dir', default=str(PARENT / 'data' / 'spare' / 'all'))
    ap.add_argument('--out_dir', default=str(ROOT / 'plots' / 'qc_test_mse'))
    ap.add_argument(
        '--view_config',
        default=str(ROOT / 'configs' / 'dvf_view_config.json'),
    )
    args = ap.parse_args()
    ...
    g = UNetCRB(im_size=128, n_phases=10).to(device)
    ...
    print(f'ckpt={args.ckpt} pairs={len(pairs)} out={out_dir}')
    ...
    print(
        f'leave-out 5/9 mean L1={np.mean([r[2] for r in loo]):.4f} '
        f'cos={np.nanmean([r[4] for r in loo]):.4f}'
    )
