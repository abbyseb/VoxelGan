"""Full-volume QC for Decoder-FiLM MSE runs. Metrics = L1 + cos."""
...
FILM_ROOT = Path(__file__).resolve().parents[1]
...
from utilities.losses import DVFLoss
...
def main():
    ...
    ap.add_argument('--held_out', default='5,9', help='1-indexed hold-out phases for summary')
    ...
    held_lab = '/'.join(f'{p}' for p in sorted(held))
    print(
