"""Dan 2.0 Decoder-FiLM: UNetFiLM + lung-masked MSE only (no discriminator).

Usage (from this directory):
  PYTHONPATH=../.. python train_film_mse.py --held_out 5,9 --run_dir LeaveOut_5_9
"""
...
FILM_ROOT = Path(__file__).resolve().parent
DAN20 = FILM_ROOT.parent
REPO = DAN20.parent
sys.path.insert(0, str(REPO))
sys.path.insert(0, str(FILM_ROOT))

from mse_loss import DVFMSELoss
...
def parse_held_out(s: str):
    """1-indexed SPARE phases, e.g. '5,9' → 0-indexed [4, 8]."""
...
def main():
    ...
    ap.add_argument('--held_out', required=True, help='1-indexed phases, e.g. 5,9 or 3,6,8')
    ap.add_argument('--run_dir', required=True, help='subdir under DecoderFiLM/, e.g. LeaveOut_5_9')
    ap.add_argument('--epochs', type=int, default=100)
    ap.add_argument('--lr', type=float, default=1e-4)
    args = ap.parse_args()

    held_1idx, held_0idx = parse_held_out(args.held_out)
    run_dir = FILM_ROOT / args.run_dir
    data_all = str(REPO / 'data' / 'spare' / 'all')
    tag = '_'.join(f'{p:02d}' for p in held_1idx)
    filename = f'film_mse_lo_{tag}'
    ...
    generator = UNetFiLM(im_size=im_size, n_phases=n_phases, int_steps=6).to(device)
    mse_loss = DVFMSELoss()
    ...
    print(
        f'[{filename}] device={device} | UNetFiLM params={n_g / 1e6:.2f}M | '
        f'loss=lung-masked MSE | no D | held_out_1idx={held_1idx}'
    )
    print(
