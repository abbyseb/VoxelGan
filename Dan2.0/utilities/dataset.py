...
class PhasePairDataset(Dataset):
    def __init__(
        ...
        # held_out_phases: 0-indexed phase ids for leave-phase-out
        # holdout_mode:
        #   'exclude' → train: drop any pair touching a held-out phase
        #   'include' → val/test: keep only pairs that touch a held-out phase
        self.held_out_phases = set(held_out_phases or [])
        if holdout_mode not in ('exclude', 'include'):
            raise ValueError(f"holdout_mode must be 'exclude' or 'include', got {holdout_mode!r}")
        ...
        for name in pair_names:
            ...
            touches = (
                ref_phase in self.held_out_phases or target_phase in self.held_out_phases
            )
            if self.holdout_mode == 'exclude' and touches:
                continue
            if self.holdout_mode == 'include' and not touches:
                continue
